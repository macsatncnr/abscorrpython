...
if __name__ == '__main__':
    ...
    startime=time.time()
    for tt in angles:
         p = multip.Process(target=hcyl.half_cylinder_sample_transm, args=(tt[0], tt[1]),kwargs=kwargs)
         jobs.append(p)
         p.start()
    for proc in jobs:
         proc.join()
